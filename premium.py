...
def menu():
    ...
    print((k+"["+p+"8"+k+"]"+p+" Change User Agent"))
...
def useragent():
    print ("\n%s[%s•%s]%s Use Default Or Manual Useragent %s[%sd/m%s]"%(k,p,k,p,h,p,h))
    pu = input("%s[%s•%s]%s Choose : "%(k,p,k,p))
    if pu in[""]:
        print ("\n%s[!] Fill In The Correct"%(m))
        useragent()
    elif pu in["d","D"]:
        os.system("rm -rf user_agent.txt")
        print ("\n%s[%s•%s] %sSuccessfully Changed User Agent"%(h,p,h,p))
        input(k+"\n[ "+p+"Back"+k+" ]"+p)
        menu()
    elif pu in["m","M"]:
        os.system("rm -rf user_agent.txt")
        uas = input("\n%s[%s•%s] %sUser Agent : "%(k,p,k,p))
        if uas == "":
            print ("\n%s[!] Fill In The Correct"%(m))
            useragent()
        try:
            ua = open('user_agent.txt','w')
            ua.write(uas)
            ua.close
            print ("\n%s[%s•%s] %sSuccessfully Changed User Agent"%(h,p,h,p))
            input(k+"\n[ "+p+"Back"+k+" ]"+p)
...
def ttll(em,pas,hosts):
	try:
		...
		ua = requests.get('https://raw.githubusercontent.com/Dapunta/premium/main/data/useragent2.txt').text.strip()
	r=requests.Session()
	r.headers.update({"Host":"mbasic.facebook.com","cache-control":"max-age=0","upgrade-insecure-requests":"1","user-agent":ua,"accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8","accept-encoding":"gzip, deflate","accept-language":"id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"})
	p=r.get("https://mbasic.facebook.com/")
	b=bs4.BeautifulSoup(p.text,"html.parser")
